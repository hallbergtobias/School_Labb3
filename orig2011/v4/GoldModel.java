...
public class GoldModel extends GameUtils implements GameModel{
	...
	private PropertyChangeSupport propertyCS = new PropertyChangeSupport(this);
	...
	public GoldModel() {
		...
		for (int i = 0; i < size.width; i++) {
			for (int j = 0; j < size.height; j++) {
				this.board[i][j] = BLANK_TILE;
			}
		}

		// Insert the collector in the middle of the gameboard.
		this.collectorPos = new Position(size.width / 2, size.height / 2);
		this.board[collectorPos.getX()][collectorPos.getY()] = COLLECTOR_TILE;
	...
	public void gameUpdate(final int lastKey) throws GameOverException {
		...
		propertyCS.firePropertyChange("GameboardState", oldCoinPos, null);
    ...
    public void addObserver(PropertyChangeListener observer) {
        propertyCS.addPropertyChangeListener(observer);
    }

    @Override
    public void removeObserver(PropertyChangeListener observer) {
        propertyCS.removePropertyChangeListener(observer);
    }
